...
一个轻量的个人博客静态站，风格参考 Hexo/NexT 类博客：左侧个人信息栏，右侧文章流，包含首页、关于、归档、搜索，以及右侧分类/标签筛选。
...
目录路径作为分类、正文前 180 个可读字符作为摘要，并记录首次发布时间与最后修改时间。
...
代码块的开头建议写语言名，系统会自动高亮、显示行号并提供复制按钮；不写语言名时会按纯文本显示：
...
链接、图片和带语言高亮的代码块，并自动生成 `assets/posts.js`。页面另外使用 KaTeX 渲染
`$...$` 与 `$$...$$` 数学公式。
